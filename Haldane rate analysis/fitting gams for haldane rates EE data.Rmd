...
```
####preamble
Here we are going to fit, for each topic, a spline to the abs.hald.num as function of interval. We are then going to divide it by the interval in order to get the haldane rate. 
...
`````
subset to just those midyears that are later than the startyears for topics
````{r}
...
min(d.2$N)
```
specify the data.  The x is interval, y is the abs.hald.number, grouping variable is topic
````{r}
y<-d$abs.hald.num
dat<-as.data.frame(y)
dat$x<-d$interval
...
`````
now we have to remove all topics with few data points.  There is one, topic 95 with only 3 intervals and another with 294. We are very interested in the other short topics such as molecular phylogenetics, so we operate to that minimum.
````{r}
dat<-subset(dat, !topic=="topic94")
``````
Now, let's look only at intervals 1: 30 since some of our topics don't have 110 years of intervals
`````{r}
dat<-subset(dat, x<=30)
checkall<- ddply(dat, .(topic), summarise,
length.haldests =length(y),
...
The function below needs the same variable lengths. But now they differ, so let's pull out a random selection of the minimum, 345. This does mean that we throw away a lot of data on the other topics, but I think that the haldane gams will still be oK.
````{r}
...
since the x (interval) variable, and the spline points along it are the same in all cases, we can cheat and specify one set of them.  We fit spline points at 10 year intervals (we have previously fitted the maximum allowed by the data) to describe how abs.hald.rate varies as a function of interval. This function forces the spline through (0,0)
```{r}
